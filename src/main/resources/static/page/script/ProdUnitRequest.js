...
function filterData() {
    ...
    filteredData = originalData.filter(item => {
        ...
        // 過濾：派工別完全符合（taskType）
        if (filters.workOrderStatus && item.taskType !== filters.workOrderStatus) return false;
...
function initializeData() {
    ...
    if (!raw) {
        document.getElementById("dataBody").innerHTML =
            `<tr><td colspan="15" style="text-align:center; color: red;">找不到資料，請重新登入</td></tr>`;
        return;
    }
    ...
    try {
        ...
        document.getElementById("dataBody").innerHTML =
            `<tr><td colspan="15" style="text-align:center; color: red;">資料格式錯誤，請重新登入</td></tr>`;
    }
}
